...
void TencentSDKWarper::onRecvVideoFrame(const char *userId, TRTCVideoStreamType streamType, TRTCVideoFrame *frame)
{
    ...
    char command[128] = {0};
    sprintf(command, "mplayer -vo xv -demuxer rawvideo -rawvideo w=%d:h=%d:format=yv12 %s", frame->width, frame->height,
            yuv_file);
...
void TencentSDKWarper::onMediaStreamAudioDecode(TRTCDecodeAudioFrame *frame)
{
    if (_bEnderRoom == true) {
        ...
        printf(
            "do not send audio timestamp %llu length %u samplerate %d channel "
            "%d \n",
...
void TencentSDKWarper::onMediaPlayerNotifyEvent(int event, void *param)
{
    ...
    if (event == EVENT_MEDIAPLAY_PLAY_SUCCESS) {
        ...
        if (_streamreader) {
            ...
            printf(
                "--- duration %lld  sduration %s fps %d video width %d video "
                "height %d --\n",
                info.duration, info.str_duration, info.videoFps, info.videoWidth, info.videoheight);
...
static void _trtc_task_start(TRTCParams params, std::string file_path, EncrytionMode mode, int av_flag)
{
...
int qcloud_iot_trtc_wrapper_start(IOT_Trtc_Params params)
{
    ...
    // set up thread
    sg_trtc_thread = std::thread(_trtc_task_start, trtc_params, std::string((char *)params.user_data),
